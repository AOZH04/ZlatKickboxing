(() => {
    ...
    function initSliders() {
        if (document.querySelector(".banner__slider")) new core(".banner__slider", {
        ...
        if (document.querySelector(".brands__slider")) {
            ...
            function updateSlideOpacity(swiper) {
                ...
                slides.forEach(((slide, index) => {
                    if (index >= activeIndex && index < activeIndex + slidesPerView) slide.style.opacity = "1"; else slide.style.opacity = "0";
                }));
            }
        }
        if (document.querySelector(".products__slider")) {
            new core(".products__slider", {
                modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
                observer: true,
                observeParents: true,
                observeSlideChildren: true,
        ...
        if (document.querySelector(".productcard__slider")) {
            new core(".productcard__slider", {
                modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
                observer: true,
                observeParents: true,
                observeSlideChildren: true,
            ...
            const sliderElements = document.querySelectorAll(".productcard__slider");
            sliderElements.forEach((sliderElement => {
                ...
                sliderElement.addEventListener("mousemove", (event => {
                    if (null !== lastMouseX) {
                        const diffX = event.clientX - lastMouseX;
                        distanceMoved += Math.abs(diffX);
                        if (distanceMoved >= moveThreshold) {
                            if (diffX > 0) swiperInstance.slideNext(); else swiperInstance.slidePrev();
                            distanceMoved = 0;
                        }
                    }
        ...
        if (document.querySelector(".productimgmin__slider")) var minSlider = new core(".productimgmin__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
            ...
            direction: "vertical",
            navigation: {
                prevEl: ".productimgmin__prev",
                nextEl: ".productimgmin__next"
            },
            keyboard: {
                enabled: true,
                onlyInViewport: true,
        ...
        if (document.querySelector(".productimgmax__slider")) var maxSlider = new core(".productimgmax__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
        ...
        if (maxSlider) {
            maxSlider.on("slideChange", (function() {
                var slideIndex = maxSlider.realIndex;
                minSlider.slideTo(slideIndex);
            }));
            minSlider.on("slideChange", (function() {
                var slideIndex = minSlider.realIndex;
                maxSlider.slideTo(slideIndex);
            }));
        }
    }
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        if (btnCatalog) {
            const parentElement = btnCatalog.parentElement;
            let documentClickListener = function(event) {
                if (parentElement && !parentElement.contains(event.target)) {
                    parentElement.classList.remove("catalogopen");
            ...
            btnCatalog.addEventListener("click", (function(event) {
                ...
                if (parentElement) {
                    const isOpen = parentElement.classList.toggle("catalogopen");
                    if (isOpen) document.addEventListener("click", documentClickListener); else document.removeEventListener("click", documentClickListener);
                }
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const elementsDel = document.querySelectorAll(".productcard_del");
        const elementsInfo = document.querySelectorAll(".productcard_info");
        elementsDel.forEach((element => {
            if ("" === element.textContent.trim()) element.style.opacity = "0";
        }));
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        if (copyButton) copyButton.addEventListener("click", (function(event) {
            event.preventDefault();
            const promoText = document.getElementById("firstequip").textContent.trim();
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        const reviewStars = document.querySelectorAll(".review_stars");
        reviewStars.forEach((function(review) {
            const quan = parseInt(review.getAttribute("data-quan"), 10);
            const starContent = review.innerHTML;
            review.innerHTML = "";
            for (let i = 0; i < quan; i++) review.innerHTML += starContent;
        }));
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const quanNumberElements = document.querySelectorAll(".quan_number");
        quanNumberElements.forEach((numberElement => {
            const value = parseInt(numberElement.textContent.trim(), 10);
            const quanlineElement = numberElement.nextElementSibling;
            if (quanlineElement && quanlineElement.classList.contains("quanline")) {
                quanlineElement.className = quanlineElement.className.replace(/\bquanline\d+\b/g, "");
                if (value >= 1 && value <= 5) quanlineElement.classList.add(`quanline${value}`);
            }
        }));
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (totalBody) {
            const quanNumberElements = totalBody.querySelectorAll(".quan_number");
            let total = 0;
            let count = 0;
            quanNumberElements.forEach((numberElement => {
                const value = parseFloat(numberElement.textContent.trim());
                if (!isNaN(value)) {
                    total += value;
                    count++;
                }
            ...
            if (count > 0) {
                ...
                const totalTopSpan = document.querySelector(".total_top span");
                if (totalTopSpan) totalTopSpan.textContent = average;
            }
        }
    }));
    document.addEventListener("afterPopupOpen", (function(e) {
        const currentPopup = e.detail.popup;
        if ("#location" === currentPopup.hash) {
            const locationLink = document.querySelector('a.menu__location_link[data-popup="#location"]');
            const dataLocElements = document.querySelectorAll("[data-loc][data-loclat]");
            dataLocElements.forEach((element => {
                element.addEventListener("click", (() => {
                    const loc = element.getAttribute("data-loc");
                    const locLat = element.getAttribute("data-loclat");
                    locationLink.textContent = loc;
                    const url = new URL(window.location);
                    url.searchParams.set("city", locLat);
                    window.history.pushState({}, "", url);
                    const allLinks = document.querySelectorAll("a");
                    allLinks.forEach((link => {
                        const linkUrl = new URL(link.href);
                        linkUrl.searchParams.set("city", locLat);
                        link.href = linkUrl.toString();
                    }));
                    if ("function" === typeof currentPopup.close) currentPopup.close();
                }));
            }));
        }
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        const cityLat = urlParams.get("city");
        if (cityLat) {
            const cityElement = document.querySelector(`[data-loclat="${cityLat}"]`);
            if (cityElement) {
                locationLink.textContent = cityElement.getAttribute("data-loc");
                const allLinks = document.querySelectorAll("a");
                allLinks.forEach((link => {
                    const linkUrl = new URL(link.href);
                    linkUrl.searchParams.set("city", cityLat);
                    link.href = linkUrl.toString();
                }));
            }
        }
